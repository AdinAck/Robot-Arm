...
class CalibrationWizard(Widget):
    ...
    def step1(self):
        ...
        self.continueButton = ttk.Button(buttons, text="Continue", command=self.step2)
        self.continueButton.pack(side="right")
        self.cancelButton = ttk.Button(buttons, text="Cancel", command=self.close)
        ...
        ttk.Label(
            self.contentFrame,
            text="""Motor 2 requires manual calibration.
    ...
    def step7(self):
        ...
        with open("config/m3", "w") as f:
            for num in self.control._system.autoCalibrate(
                self.control._system.m_outer_rot, speed=2
            ):
                f.write(f"{num}\n")
